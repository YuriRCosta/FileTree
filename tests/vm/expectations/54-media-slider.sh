...
script = Path('tests/vm/image-gallery-drag.toml').read_text().replace('gallery-pointer', 'brindle-media-slider').replace('START_X', str(x)).replace('START_Y', str(y)).replace('END_X', str(end)).replace('END_Y', str(y))
...
control('openBlade', 'left')
print(f'{checks} media slider checks passed', flush=True)
PY
