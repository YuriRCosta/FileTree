...
TestCase {
  ...
  function test_seek_drill_and_keyboard_leave_content_filter_alone() {
    ...
    compare(seekSpy.count, 1)
    var index = view.detail.bins.findIndex(function(bin) { return bin.count > 0 })
    view.seek(index, 0)
    keyClick(Qt.Key_Right)
  ...
  function test_pointer_seek_is_immediate_empty_bins_are_inert() {
    var view = create()
    ...
    mouseMove(view, 50, view.axisTop + view.rowHeight * 5.5)
  ...
  function test_outline_drag_seeks_without_hover_geometry_changes() {
    ...
    mouseMove(view, 50, view.axisTop + view.rowHeight * 11.5, 20)
    verify(seekSpy.count > 0)
    verify(seekSpy.signalArguments[seekSpy.count - 1][0] > 3000)
    mouseRelease(view, 50, view.axisTop + view.rowHeight * 11.5)
