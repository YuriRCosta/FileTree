...
TestCase {
  ...
  function test_discovery_is_lazy_and_explicit_connect_uses_saved_fields_without_udisks() {
    ...
    compare(requests.length, 1)
    controller.connectPeer("tailnet:peer", "user", "/files", true)
    compare(requests[1].command, "location-connect")
    compare(requests[1].arguments, ["--location", "tailnet:peer", "--user", "user", "--path", "/files", "--save"])
    controller.connectPeer("tailnet:peer", "user", "/files", true)
  ...
  function test_old_inventory_cannot_replace_action_result_and_cleanup_stays_disconnectable() {
    ...
    controller.connectPeer("tailnet:peer", "user", "/files", false)
