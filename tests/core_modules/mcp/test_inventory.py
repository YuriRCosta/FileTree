...
class QmlContractCase(unittest.TestCase):
    def test_core_module_and_shared_tree_contract(self) -> None:
        core = Path(__file__).resolve().parents[3]
        root = core / "modules" / "mcp"
        ...
        self.assertFalse((root / "Service.qml").exists())
        self.assertFalse((root / "HostGuard.qml").exists())
        ...
        self.assertIn("InventoryProvider {", provider)
        shared_provider = (root.parents[1] / "ui/InventoryProvider.qml").read_text()
        self.assertIn('observers: Qt.binding(function() { return provider.observers })', shared_provider)
        ...
        cli = (core / "python" / "agent_mcp" / "cli.py").read_text(encoding="utf-8")
