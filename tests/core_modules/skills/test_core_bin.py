...
KIND = "skills"
DESCRIPTOR = "---\nname: rivet-fixture\ndescription: a fixture skill for the recoverable bin\n---\n\nbody\n"
...
class CoreBinLifecycle(unittest.TestCase):
    def setUp(self):
        ...
        self.descriptor = self.fixture / "SKILL.md"
        self.fixture.mkdir(parents=True)
        self.descriptor.write_text(DESCRIPTOR)
    ...
    def listed(self):
        ...
        rows = [item for item in document["items"] if str(item.get("path")) == str(self.descriptor)]
        self.assertEqual(len(rows), 1, document)
        return rows[0]

    def item(self):
        row = self.listed()
        return json.dumps({"id": row["id"], "name": row.get("name", ""), "kind": "skill",
                           "scope": row.get("scope", ""), "detail": "", "path": str(self.fixture), "isDir": True,
                           "paths": [str(self.fixture)], "skillRoot": True, "position": 0, "groups": ["User"]})
    ...
    def test_delete_moves_the_skill_into_the_bin_and_restore_puts_it_back_unchanged(self):
        entry = self.remove()
        self.assertFalse(self.fixture.exists())
        rows = self.entries()
        self.assertEqual([row["id"] for row in rows], [entry])
        self.assertEqual(rows[0]["kind"], "bin")
        self.assertEqual(rows[0]["name"], "rivet-fixture")
        ...
        self.assertTrue(response.get("ok"), response)
        self.assertEqual(self.descriptor.read_text(), DESCRIPTOR)
        self.assertEqual(self.entries(), [])
        self.assertEqual(self.listed()["path"], str(self.descriptor))

    def test_discard_is_the_only_step_that_destroys_the_skill(self):
        entry = self.remove()
    ...
    def test_removal_is_refused_and_the_skill_untouched_while_management_is_disabled(self):
        item = self.item()
        self.consent(False)
