...
SOURCES = ["manifest.json", "Cargo.toml", "Cargo.lock", "CHANGELOG.md", "filetree-bin"]
...
class VersionHookTests(unittest.TestCase):
    ...
    def write_versions(self, value="0.1.3"):
        ...
        self.write_binary(f'printf "filetree {value}\\n"')

    def write(self, name, content):
        (self.repo / name).write_text(content)

    def write_binary(self, body):
        self.write("filetree-bin", f"#!/usr/bin/env bash\n{body}\n")
        (self.repo / "filetree-bin").chmod(0o755)
    ...
    def hook(self, success=True, contains=""):
        ...
        self.assertEqual(list((self.repo / ".git").glob("filetree-version-*")), [])
        return result
    ...
    def test_shared_hook_works_inside_a_linked_worktree(self):
        ...
        self.assertEqual(list(git_directory.glob("filetree-version-*")), [])
        self.repo = primary
    ...
    def test_missing_or_symlinked_sources_are_rejected(self):
        for name in SOURCES:
            with self.subTest(name=name):
                ...
                if name == "filetree-bin":
                    (self.repo / name).chmod(0o755)
                self.git("add", "--", name)

    def test_nonexecutable_staged_binary_is_rejected(self):
        self.git("update-index", "--chmod=-x", "filetree-bin")
        self.hook(False, "executable mode 100755")
        self.assertTrue(os.access(self.repo / "filetree-bin", os.X_OK))

    def test_binary_failures_and_versions(self):
        for body in ["exit 1", "exit 0", "echo broken", "echo filetree 0.9.9", "echo other 0.1.3", "echo filetree 0.1.3; echo extra"]:
            with self.subTest(body=body):
                self.write_binary(body)
                self.git("add", "filetree-bin")
                self.write_binary('echo filetree 0.1.3')
                self.hook(False)
