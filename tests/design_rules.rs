...
fn every_registered_exception_names_a_live_surface_and_a_reason() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let registry = std::fs::read_to_string(root.join("tools/design-exceptions.json")).expect("registry");
    let document: serde_json::Value = serde_json::from_str(&registry).expect("registry json");
    let entries = document["no-rounded-corners"].as_array().expect("rule entries");
    ...
    for entry in entries {
        ...
        assert!(reason.len() > 12, "{path}: an exception needs a real reason");
