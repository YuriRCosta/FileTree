...
- FileTree now uses its own identifiers: the Omarchy plugin id `yuricosta.filetree` (IPC targets `yuricosta.filetree` and `yuricosta.filetree.control`), the `filetree` command and `filetree-bin` backend, `FILETREE_*` environment variables, `filetree:` global shortcuts, and the `omarchy/filetree` config and state directories. Existing `omarchy/fileblade` directories move to the new names on first start; the thumbnail cache starts fresh.
- The project is renamed FileTree, at https://github.com/YuriRCosta/FileTree. The interface, CLI messages and documentation say FileTree.
...
- The native app, its installer and packaging, the file chooser portal, desktop roles and the native state migration are removed; FileBlade runs only as an Omarchy plugin.
