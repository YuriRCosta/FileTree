...
function zoomStep(key, modifiers) {
  if (!(modifiers & Qt.ControlModifier) || (modifiers & (Qt.AltModifier | Qt.MetaModifier))) return null
  if (key === Qt.Key_Plus || key === Qt.Key_Equal) return 1
  if (key === Qt.Key_Minus || key === Qt.Key_Underscore) return -1
  if (key === Qt.Key_0) return 0
  return null
}

function zoomed(scale, step) {
  if (step === 0) return 1.0
  return scaleFromPercent(percentFromScale(scale) + step * PERCENT_STEP)
}
