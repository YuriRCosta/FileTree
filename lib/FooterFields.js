...
function window(parts, offset, widthOf, budget, gap) {
  ...
  return {
    shown: shown,
    start: start,
    more: start + shown.length < parts.length,
    back: start > 0
  }
}
...
function retreat(parts, start, shownCount) {
  var previous = start - Math.max(1, shownCount)
  return previous < 0 ? Math.max(0, parts.length - Math.max(1, shownCount)) : previous
}
