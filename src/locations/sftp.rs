...
mod tests {
    ...
    fn options() -> crate::backend::LocationListArgs {
        ...
        .unwrap()
        {
            crate::backend::BackendCommand::List(options) => options,
            _ => unreachable!(),
        }
    }

    #[test]
    fn saved_location_has_only_host_user_path_and_encodes_literal_remote_names() {
        let saved = Saved {
            host: "peer.tail.test".into(),
            user: "user".into(),
            path: "/space #percent%/雪".into(),
        };
        let uri = saved.uri().unwrap();
        assert!(uri.contains("space%20%23percent%25"), "{uri}");
        assert!(uri.contains("%E9%9B%AA"), "{uri}");
        let document = serde_json::to_value(&saved).unwrap();
        assert_eq!(document.as_object().unwrap().len(), 3);
        assert!(
            serde_json::from_value::<Saved>(
                json!({"host":"peer","user":"user","path":"/","password":"secret"})
            )
            .is_err()
        );
    ...
    #[test]
    fn listing_decodes_names_but_rejects_other_authorities_and_parent_paths() {
        let parent = url::Url::parse("sftp://user@host/root/").unwrap();
        let rows = parse_listing(
            "sftp://user@host/root/a%0Ab%25\t4\t(regular)\ttime::modified=1700000000\n",
            &parent,
            &options(),
        )
        .unwrap();
        assert_eq!(rows["entries"][0]["name"], "a\nb%");
        assert_eq!(rows["entries"][0]["modified"], "2023-11-14T22:13:20Z");
        for uri in [
            "file:///root/name",
            "sftp://user@other/root/name",
            "sftp://user@host/elsewhere/name",
            "sftp://user@host/root/a%2Fb",
        ] {
            assert!(parse_listing(&format!("{uri}\t4\t(regular)\n"), &parent, &options()).is_err());
        }
        assert!(parse_listing("broken record", &parent, &options()).is_err());
    }
}
